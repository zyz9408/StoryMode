...
export function emptyAnswerError({message={},finishReason,hasReasoning=false,hasTools=false,hasRefusal=false}={}) {
  ...
  if(hasReasoning || message.reasoning_content || message.reasoning || message.thinking || (Array.isArray(message.content)&&message.content.some(p=>p?.thought||['thinking','reasoning'].includes(p?.type))))return new Error('模型仅返回推理内容，没有最终正文。请检查输出 token 上限或降低推理强度后继续；推理内容不会当作小说。');
