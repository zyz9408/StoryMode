...
export function PresetManager({presets,story,refresh,refreshStory}:{presets:Preset[];story:Story|null;refresh:()=>Promise<void>;refreshStory:()=>Promise<void>}) {
  ...
  return <div className="preset-manager">
    ...
    {draft&&<fieldset disabled={busy}>
      ...
      <label>最大输出 tokens（留空使用供应商默认值）<input type="number" min={1} step={1} value={typeof draft.parameters.openai_max_tokens==='number'?draft.parameters.openai_max_tokens:''} onChange={e=>{const parameters={...draft.parameters};if(e.target.value==='')delete parameters.openai_max_tokens;else parameters.openai_max_tokens=Number(e.target.value);setDraft({...draft,parameters});setPreview(null);}}/></label>
      <p className="hint">遇到 HTTP 422 时，可根据报错字段调整输出上限，或取消“同时应用采样参数”后保存重试。负数随机种子会自动省略；DeepSeek 不接收的扩展采样参数在发送时过滤，原预设数据保留。</p>
