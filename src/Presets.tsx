  ...
  return <div className="preset-manager">
    ...
    {!draft?<div className="editor-empty"><h3>从一个预设开始</h3><p>导入 SillyTavern 预设 JSON，提示词与内嵌正则会一起读取。导入后可在这里分别管理。</p></div>:<fieldset disabled={busy}>
      ...
      <section role="tabpanel" id={`preset-panel-${tabs.indexOf(tab)}`} aria-labelledby={`preset-tab-${tabs.indexOf(tab)}`} className="editor-panel">
        ...
        {tab==='正则'&&<><label className="preset-import">导入正则 JSON（也可选择完整预设文件）<input aria-label="导入正则 JSON" type="file" accept=".json,application/json" onChange={e=>{const file=e.target.files?.[0];if(!file)return;e.target.value='';void act(async()=>{await save();const before=draft.regexScripts?.length||0;const p=await request<Preset>(`/presets/${id}/regex/import`,{source:await file.text()});await refresh();setDraft(p);setPreview(null);setMessage(`已读取并保存 ${(p.regexScripts?.length||0)-before} 条正则。`);});}}/></label><RegexEditor key={id} scripts={draft.regexScripts||[]} change={regexScripts=>update({...draft,regexScripts})}/></>}
