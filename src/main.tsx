...
function Settings({ profiles, refresh, close }: { profiles:Profile[]; refresh:()=>Promise<void>; close:()=>void }) {
  ...
  const action = async (kind:'save'|'models'|'text') => {
  ...
  return <Modal title="模型连接" subtitle="文字与插图，各自选择合适的模型。" close={close}>
    ...
    <form onSubmit={e=>{e.preventDefault();void action('save');}}><fieldset disabled={busy}>
      ...
      <p className="hint">应用不主动发起联网搜索，直接使用模型回答。生图支持 Images 及 Gemini 原生图片响应。</p>
      <div className="actions wrap"><button className="primary" type="submit">{busy?'处理中…':'保存配置'}</button><button type="button" className="secondary" onClick={()=>void action('text')}>测试文字连接</button></div>
...
function NewStory({ profiles, presets, close, created, openSettings }: { profiles:Profile[]; presets:Preset[]; close:()=>void; created:(id:string)=>void; openSettings:()=>void }) {
  ...
  return <Modal title="从一个「假如」开始" subtitle="给世界一个不同的起点，看看故事会走向哪里。" close={close}><form onSubmit={create}><fieldset disabled={busy}>
    ...
    <div className="creation-note"><Icon name="book"/><div><strong>15～30 章 · 每章 3000～8000 字</strong><p>随故事自然收束，在关键节点由你决定。长篇生成会多次调用模型并产生供应商费用。</p></div></div>
...
function Binding({ story, profiles, close, submit }: { story:Story; profiles:Profile[]; close:()=>void; submit:(data:unknown)=>Promise<void> }) {
  const [form,setForm]=useState({textProfile:story.textProfile,researchProfile:story.researchProfile,imageProfile:story.imageProfile,imageModel:story.imageModel||'',autoImages:story.autoImages!==false});
  const [busy,setBusy]=useState(false), [error,setError]=useState('');
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>{browserMode&&<div className="notice browser-notice">浏览器版 · 故事保存在当前浏览器，清除网站数据会删除故事，请及时导出。生成时请保持页面打开，刷新后手动继续。模型接口须使用 HTTPS 并允许跨域（CORS）；密钥仅保留在当前标签页会话中。</div>}<header className="topbar"><div className="breadcrumb">工作空间 <span>/</span> {active?'故事阅读':'模拟书架'}</div><div className="topbar-actions"><span className="desktop-only">由你的模型驱动</span><button className="theme-button" onClick={()=>setDark(!dark)} aria-label="切换阅读主题">{dark?'☀':'☾'}</button></div></header>
      ...
      {!active?<div className="library"><section className="hero"><div className="hero-copy"><div className="eyebrow"><span/> A DIFFERENT TURN OF HISTORY</div><h1>如果那一天，<br/>世界走向了另一边。</h1><p>从一个假如出发，让人物、时代与现实相遇。<br/>在长篇故事里，亲历选择之后的每一次回响。</p><button className="primary" onClick={()=>setModal('new')}>写下你的第一个假如 <Icon name="arrow" size={18}/></button><div className="hero-footnote">有据可循的世界 <i/> 由你决定的转折</div></div><div className="hero-art" aria-hidden="true"><div className="orbit orbit-one"/><div className="orbit orbit-two"/><div className="sun-disc"/><div className="art-mountain mountain-back"/><div className="art-mountain mountain-front"/><div className="art-line"/><div className="art-seal">未竟之史</div><span className="art-caption">ONE PREMISE. MANY POSSIBILITIES.</span><div className="art-coordinate">叁国 · 异闻录<br/>卷之〇一</div></div></section>
        <div className="feature-strip"><div><span>01</span><Icon name="globe"/><p>依据现实推演<small>由模型处理背景与人物逻辑</small></p></div><div><span>02</span><Icon name="book"/><p>15–30 章完整故事<small>每章 3000–8000 字，随因果收束</small></p></div><div><span>03</span><Icon name="leaf"/><p>关键时刻，由你选择<small>每个决定都有代价与后果</small></p></div></div>
        ...
        <div className="reader-heading"><div><div className="eyebrow">SIMULATION / {story.name}</div><h1>{story.title}</h1><div className="story-tags"><span className={'status '+story.status}>{statuses[story.status]}</span><span>模型直接推演</span><span>{story.chapters.length} / {story.plannedChapters||'15–30'} 章</span></div></div><div className="actions wrap"><button className="secondary" onClick={()=>setModal('presets')}>{story.presetEnabled?'预设已启用':'写作预设设置'}</button><button className="secondary" onClick={()=>setModal('character')}>主角定制</button><button className="secondary" disabled={busy} onClick={()=>command('illustrations/retry')}>补齐章节插图</button>{['done','evaluation'].includes(story.phase)&&story.chapters.length>=15&&<button className="secondary" disabled={busy||story.running||!!story.rewrite} onClick={()=>{setChapterTarget(story.chapters.at(-1)!.number);setRewriteInstruction('');setError('');setModal('ending');}}>补全终局</button>}<a className="secondary icon-button" title="导出 Markdown" aria-label="导出 Markdown" href={`/api/stories/${story.id}/export`} onClick={e=>{e.preventDefault();void exportFile(`/stories/${story.id}/export`).catch(e=>setError(e.message));}}><Icon name="download"/></a><button className="secondary icon-button" title="本次模拟模型" aria-label="本次模拟模型" disabled={story.running} onClick={()=>setModal('binding')}><Icon name="settings"/></button>{story.running?<button className="secondary" disabled={busy} onClick={()=>command('pause')}><Icon name="pause" size={16}/>暂停</button>:['paused','failed'].includes(story.status)?<button className="primary" disabled={busy} onClick={()=>command('resume')}><Icon name="play" size={16}/>{story.rewrite?'继续重写':'继续推演'}</button>:null}</div></div>
    ...
    {(modal==='rewrite'||modal==='ending')&&story&&targetChapter&&<Modal title={modal==='ending'?'补全人生与时代的终局':`重新生成第 ${targetChapter.number} 章`} subtitle={modal==='ending'?'保留已发生事件，将最后一章改写为终局综述：人物晚年至死亡、组织归宿、时代落幕与后人评价。审核通过后替换原结尾并更新结局评价，不增加章数。':'重写叙述、对话与节奏，保留本章关键事件与结局，保持后续剧情一致。原文在审核通过后才替换。'} close={close}><form onSubmit={e=>{e.preventDefault();void act(()=>api(modal==='ending'?`/stories/${story.id}/complete-ending`:`/stories/${story.id}/chapters/${chapterTarget}/regenerate`,{instruction:rewriteInstruction})).then(close).catch(()=>{});}}><label>这次希望怎么写？<textarea rows={4} maxLength={2000} value={rewriteInstruction} onChange={e=>setRewriteInstruction(e.target.value)} placeholder="例如：压缩无用细节，用行动和对话表现冲突，少写反复的心理描写。"/></label><p className="hint">如正在续写，会先暂停。重写完成后可继续推演。</p>{error&&<p className="notice danger" role="alert">{error}</p>}<button className="primary" disabled={busy||!!story.rewrite}>{modal==='ending'?'开始补全终局':'开始重新生成'}</button></form></Modal>}
