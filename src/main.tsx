...
import { ModelErrorDetails, type ModelDiagnostic } from './ModelErrorDetails';
...
function Modal({ title, subtitle, close, children, wide=false }: { title:string; subtitle?:string; close:()=>void; children:React.ReactNode; wide?:boolean }) {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Tab') {
        ...
        const visible=Array.from(elements).filter(el=>el.getClientRects().length);if(!visible.length)return;
  ...
  return <div className="overlay" onMouseDown={e => { if (e.target === e.currentTarget) close(); }}><div className={wide?"modal modal-wide":"modal"} ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title}><header><div><div className="eyebrow">STORYMODE / WORKSPACE</div><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div><button className="icon-button" aria-label="关闭" onClick={close}><Icon name="close"/></button></header>{children}</div></div>;
}
...
function ThemeIdeas({ profileId, choose }: { profileId:string; choose:(v:string)=>void }) {
  ...
  const [diagnostics,setDiagnostics]=useState<ModelDiagnostic|null>(null);
  return <div className="theme-ideas"><div className="theme-heading"><strong>让 AI 帮你想一个假如</strong><span>使用下方选择的文字模型</span></div><div className="inline-field"><input aria-label="主题方向" maxLength={2000} placeholder="可选方向：三国、商业、普通人穿越、科技变革…" value={direction} onChange={e=>setDirection(e.target.value)}/><button type="button" className="secondary" disabled={busy||!profileId} onClick={async()=>{setBusy(true);setError('');setDiagnostics(null);try{const r=await api<{topics:typeof ideas}>('/topics',{profileId,direction,previous:ideas.map(i=>i.event)});setIdeas(r.topics);}catch(e){setError((e as Error).message);setDiagnostics((e as Error & {details?:ModelDiagnostic}).details||null);}finally{setBusy(false);}}}>{busy?'构思中…':ideas.length?'换一批主题':'AI 生成主题'}</button></div>{!profileId&&<p className="hint">先选择一个已配置的文字模型。</p>}{error&&<ModelErrorDetails message={error} details={diagnostics}/> }<div className="idea-grid">{ideas.map(idea=><button type="button" className="idea-card" key={idea.title} onClick={()=>choose(idea.event)}><strong>{idea.title}</strong><p>{idea.event}</p><small>{idea.angle}</small><span>使用这个主题 ↗</span></button>)}</div></div>;
}
...
function NewStory({ profiles, presets, close, created, openSettings }: { profiles:Profile[]; presets:Preset[]; close:()=>void; created:(id:string)=>void; openSettings:()=>void }) {
  ...
  return <Modal title="从一个「假如」开始" subtitle="给世界一个不同的起点，看看故事会走向哪里。" close={close}><form onSubmit={create}><fieldset disabled={busy}>
    ...
    <div className="creation-note"><Icon name="book"/><div><strong>自然完结，最多 30 章 · 每章 至少3000 字</strong><p>随故事自然收束，在关键节点由你决定。长篇生成会多次调用模型并产生供应商费用。</p></div></div>
...
function App() {
  ...
  const [errorDetails,setErrorDetails]=useState<ModelDiagnostic|null>(null),[detailMessage,setDetailMessage]=useState('');
  ...
  const act=async(action:()=>Promise<unknown>)=>{setBusy(true);setError('');setErrorDetails(null);try{await action();await Promise.all([refreshStory(),refreshBooks()]);}catch(e){setError((e as Error).message);setErrorDetails((e as Error & {details?:ModelDiagnostic}).details||null);setDetailMessage((e as Error).message);throw e;}finally{setBusy(false);}};
  ...
  return <div className={'app-shell'+(sidebarCollapsed?' sidebar-collapsed':'')}>
    ...
    <main><header className="topbar"><div className="breadcrumb">工作空间 <span>/</span> {active?'故事阅读':'模拟书架'}</div><div className="topbar-actions"><span className="desktop-only">由你的模型驱动</span><button className="theme-button" onClick={()=>setDark(!dark)} aria-label="切换阅读主题">{dark?'☀':'☾'}</button></div></header>
      ...
      {error&&error===detailMessage&&errorDetails&&<ModelErrorDetails message={error} details={errorDetails}/>}
      {!active?<div className="library"><section className="hero"><div className="hero-copy"><div className="eyebrow"><span/> A DIFFERENT TURN OF HISTORY</div><h1>如果那一天，<br/>世界走向了另一边。</h1><p>从一个假如出发，让人物、时代与现实相遇。<br/>在长篇故事里，亲历选择之后的每一次回响。</p><button className="primary" onClick={()=>setModal('new')}>写下你的第一个假如 <Icon name="arrow" size={18}/></button><div className="hero-footnote">有据可循的世界 <i/> 由你决定的转折</div></div><div className="hero-art" aria-hidden="true"><div className="orbit orbit-one"/><div className="orbit orbit-two"/><div className="sun-disc"/><div className="art-mountain mountain-back"/><div className="art-mountain mountain-front"/><div className="art-line"/><div className="art-seal">未竟之史</div><span className="art-caption">ONE PREMISE. MANY POSSIBILITIES.</span><div className="art-coordinate">叁国 · 异闻录<br/>卷之〇一</div></div></section>
        <div className="feature-strip"><div><span>01</span><Icon name="globe"/><p>依据现实推演<small>由模型处理背景与人物逻辑</small></p></div><div><span>02</span><Icon name="book"/><p>章数随故事自然收束<small>每章 至少3000 字，随因果收束</small></p></div><div><span>03</span><Icon name="leaf"/><p>关键时刻，由你选择<small>每个决定都有代价与后果</small></p></div></div>
        ...
        {story.error&&<ModelErrorDetails message={story.error} details={story.errorDetails}/> }
        {story.rewrite&&<div className="notice rewrite-status"><span>第 {story.rewrite.number} 章正在重写，当前仍可阅读原文。暂停或失败后可点击继续重写。</span><button className="secondary" disabled={busy} onClick={()=>command('cancel-rewrite')}>取消重写，保留原文</button><details><summary>查看重写草稿</summary><div className="prose draft-prose"><StoryContent text={live||story.rewrite.partial||story.rewrite.body||'正在准备…'} fontSize={font} paragraphs={false} title="重写草稿样式"/></div></details></div>}
    ...
    {modal==='presets'&&<Modal wide title="预设与正则" subtitle="管理提示词编排、正则规则与模型参数。" close={close}><PresetManager presets={presets} story={story} refresh={refreshPresets} refreshStory={refreshStory}/></Modal>}
