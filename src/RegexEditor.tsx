...
export function RegexEditor({scripts,change}:{scripts:RegexScript[];change:(scripts:RegexScript[])=>void}) {
  ...
  return <details><summary>正则脚本 · {scripts.length} 条</summary>
    {scripts.map((script,index)=><details key={index} className="preset-entry"><summary>{script.disabled?'已关闭 · ':''}{script.scriptName}</summary>
      ...
      <label>查找表达式<textarea value={script.findRegex} onChange={e=>update(index,{findRegex:e.target.value})}/></label>
      <label>替换内容<textarea value={script.replaceString} onChange={e=>update(index,{replaceString:e.target.value})}/></label>
