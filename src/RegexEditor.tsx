...
export function RegexEditor({scripts,change}:{scripts:RegexScript[];change:(scripts:RegexScript[])=>void}) {
  ...
  const index=Math.min(selected,Math.max(0,scripts.length-1)),script=scripts[index];
  const update=(at:number,patch:Partial<RegexScript>)=>{setResult(null);change(scripts.map((s,i)=>i===at?{...s,...patch}:s));};
  const move=(offset:number)=>{const next=[...scripts];[next[index],next[index+offset]]=[next[index+offset],next[index]];change(next);setSelected(index+offset);};
  const add=()=>{setSelected(scripts.length);setSearch('');setResult(null);change([...scripts,{id:crypto.randomUUID(),scriptName:'新正则',findRegex:'',replaceString:'',trimStrings:[],placement:[2],disabled:false,markdownOnly:false,promptOnly:false,runOnEdit:false,substituteRegex:0,minDepth:null,maxDepth:null}]);};
  ...
  return <section aria-label="正则管理">
    ...
    {!script?<div className="editor-empty"><h3>暂无正则</h3><p>手动添加一条规则，或从独立正则文件、完整预设 JSON 中读取。</p></div>:<div className="editor-columns">
      <aside className="editor-list-pane"><label>搜索正则<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="按名称筛选"/></label><div className="editor-list" aria-label="正则列表">
        {scripts.map((s,i)=>s.scriptName.toLowerCase().includes(search.toLowerCase())&&<div className={'editor-list-row '+(i===index?'selected':'')} key={`${s.id||''}-${i}`}><input type="checkbox" aria-label={`启用正则 ${s.scriptName}`} checked={!s.disabled} onChange={e=>update(i,{disabled:!e.target.checked})}/><button type="button" aria-pressed={i===index} onClick={()=>{setSelected(i);setResult(null);}}><strong>{s.scriptName||'未命名正则'}</strong><small>{i+1}. {modeLabel(s)} · {s.disabled?'已停用':'已启用'}</small></button></div>)}
        {!scripts.some(s=>s.scriptName.toLowerCase().includes(search.toLowerCase()))&&<p className="hint">没有匹配的正则。</p>}
      </div></aside>
      <div className="editor-detail"><div className="editor-detail-title"><h3>编辑规则</h3><span className="editor-badge">{modeLabel(script)}</span></div>
        ...
        <details className="regex-test"><summary>测试当前规则</summary><p className="hint">只测试当前规则，遵守启停、作用位置和深度设置，不修改故事。宏使用示例角色与临时变量。</p><label>测试文本<textarea aria-label="测试文本" rows={4} value={sample} onChange={e=>{setSample(e.target.value);setResult(null);}}/></label>
          <div className="form-grid"><label>测试阶段<select value={testMode} onChange={e=>{setTestMode(e.target.value);setResult(null);}}><option value="stored">修改实际文本</option><option value="prompt">发送提示词</option><option value="display">阅读显示</option></select></label><label>测试位置<select value={placement} onChange={e=>{setPlacement(Number(e.target.value));setResult(null);}}><option value={1}>用户输入</option><option value={2}>AI 输出</option><option value={5}>世界信息</option><option value={6}>推理</option></select></label><label>测试深度<input type="number" min={0} value={depth} onChange={e=>{setDepth(Number(e.target.value));setResult(null);}}/></label></div>
          <button type="button" className="secondary" disabled={!!invalid} onClick={()=>setResult(applyRegex(sample,[script],placement,{isPrompt:testMode==='prompt',isMarkdown:testMode==='display',depth,substitute:createMacroEnvironment({player:'玩家',protagonist:{name:'主角'}}).substitute}))}>运行测试</button>{result!==null&&<div className="regex-test-result"><h4>测试结果</h4><pre>{result||'〔空文本〕'}</pre>{result===sample&&<p className="hint">文本未改变：请检查匹配内容、启停和测试阶段。</p>}</div>}
        </details>
        <div className="actions wrap editor-item-actions"><button type="button" className="secondary" disabled={index===0} onClick={()=>move(-1)}>上移正则</button><button type="button" className="secondary" disabled={index===scripts.length-1} onClick={()=>move(1)}>下移正则</button><button type="button" className="secondary" onClick={()=>{change(scripts.filter((_,i)=>i!==index));setResult(null);}}>移除此正则</button></div>
      </div>
    </div>}
  </section>;
}
