...
export function fixture(task, ctx, options = {}) {
  ...
  if(task.startsWith('重新规划后续故事')) return {plannedChapters:ending,outline:Array.from({length:ending-ctx.regeneration.from+1},(_,i)=>({number:ctx.regeneration.from+i,title:`重新推演${ctx.regeneration.from+i}`,purpose:'沿新因果推进'})),decisionChapters:options.noDecisions?[]:[3,7,11].filter(n=>n>=ctx.regeneration.from&&n<ending)};
  ...
  if(task.startsWith('根据实际完成章节')) return { scorecard:{title:'有限资源生存家',summary:'没能改写天下，至少把日子过明白了。',cards:['主角','势力','朋友','对手','时代影响'].map((category,i)=>({category,name:'旅人与商队',score:80-i*5,title:'精打细算的一生',comment:'付出饮料换取情报，守住有限物资。',chapters:[options.badCitation?31:1]}))}, conclusion:'测试结局：在有限资源下落脚，未改变整体历史格局。', dimensions:dimensionNames.map(name=>({name,assessment:'基于第一章有限物资交换行为评价，结局属于推演。',chapters:[options.badCitation?31:1]})), uncertainties:'模拟测试数据；不构成历史考据或文学质量示例。' };
